...
class ACMDataset(DGLDataset):
    ...
    * feat: tensor(72, 72) one-hot编码
    ...
    def process(self):
        ...
        p_vs_p = data['PvsP']  # papper-paper citation
        ...
        self.g = dgl.heterograph({
            ('paper', 'citation', 'paper'): p_vs_p.nonzero(),
...
class ACM3025Dataset(DGLDataset):
    ...
    * train_mask, val_mask, test_mask: tensor(3025)
    ...
    def process(self):
        ...
        labels = torch.from_numpy(data['label'].todense()).long().nonzero(
            as_tuple=True)[1]  # (3025)
