...
class DBLPFourAreaDataset(DGLDataset):
    ...
    * label: tensor(20)，类别为0~3
    ...
    def process(self):
        ...
        # if process and save as .bin before, they will call load funciton directly
    ...
    def _read_raw_data(self):
        authors = self._read_file('author_label.txt', names=[
                                  'id', 'label', 'name'], index_col='id')
        papers = self._read_file(
            'paper.txt', names=['id', 'title'], index_col='id', encoding='cp1252')
        confs = self._read_file('conf_label.txt', names=[
                                'id', 'label', 'name', 'dummy'], index_col='id')
        terms = self._read_file(
            'term.txt', names=['id', 'name'], index_col='id')
        paper_author = self._read_file('paper_author.txt', names=[
                                       'paper_id', 'author_id'])
        paper_conf = self._read_file('paper_conf.txt', names=[
                                     'paper_id', 'conf_id'])
        paper_term = self._read_file('paper_term.txt', names=[
                                     'paper_id', 'term_id'])
    ...
    def _filter_nodes_and_edges(self):
        ...
        self.paper_conf = self.paper_conf[self.paper_conf['paper_id'].isin(
            paper_ids)]
        self.paper_term = self.paper_term[self.paper_term['paper_id'].isin(
            paper_ids)]
    ...
    def _remove_stopwords(self):
        ...
        self.paper_term = self.paper_term[self.paper_term['term_id'].isin(
            self.terms.index)]
    ...
    def _reset_index(self):
        ...
        author_id_map = {row['id']: index for index,
                         row in self.authors.iterrows()}
        paper_id_map = {row['id']: index for index,
                        row in self.papers.iterrows()}
        conf_id_map = {row['id']: index for index,
                       row in self.confs.iterrows()}
        term_id_map = {row['id']: index for index,
                       row in self.terms.iterrows()}

        self.paper_author.loc[:, 'author_id'] = [author_id_map[i]
                                                 for i in self.paper_author['author_id'].to_list()]
        self.paper_conf.loc[:, 'conf_id'] = [conf_id_map[i]
                                             for i in self.paper_conf['conf_id'].to_list()]
        self.paper_term.loc[:, 'term_id'] = [term_id_map[i]
                                             for i in self.paper_term['term_id'].to_list()]
        for df in (self.paper_author, self.paper_conf, self.paper_term):
            df.loc[:, 'paper_id'] = [paper_id_map[i] for i in df['paper_id']]

    def _build_graph(self):
        pa_p, pa_a = self.paper_author['paper_id'].to_list(
        ), self.paper_author['author_id'].to_list()
        pc_p, pc_c = self.paper_conf['paper_id'].to_list(
        ), self.paper_conf['conf_id'].to_list()
        pt_p, pt_t = self.paper_term['paper_id'].to_list(
        ), self.paper_term['term_id'].to_list()
    ...
    def _add_ndata(self):
        ...
        self.g.nodes['author'].data['feat'] = torch.from_numpy(
            mat['features']).float()
        self.g.nodes['author'].data['label'] = torch.tensor(
            self.authors['label'].to_list())

        n_authors = len(self.authors)
        train_idx, val_idx, test_idx = self.split_idx(
            np.arange(n_authors), 800, 400, self._seed)
        self.g.nodes['author'].data['train_mask'] = generate_mask_tensor(
            idx2mask(train_idx, n_authors))
        self.g.nodes['author'].data['val_mask'] = generate_mask_tensor(
            idx2mask(val_idx, n_authors))
        self.g.nodes['author'].data['test_mask'] = generate_mask_tensor(
            idx2mask(test_idx, n_authors))

        self.g.nodes['conf'].data['label'] = torch.tensor(
...
class DBLP4057Dataset(DGLDataset):
    ...
    * train_mask, val_mask, test_mask: tensor(4057)
    ...
    def save(self):
        save_graphs(os.path.join(self.save_path,
                    self.name + '_dgl_graph.bin'), self.gs)

    def load(self):
        self.gs, _ = load_graphs(os.path.join(
            self.save_path, self.name + '_dgl_graph.bin'))
        for g in self.gs:
            for k in ('train_mask', 'val_mask', 'test_mask'):
    ...
    def process(self):
        data = sio.loadmat(os.path.join(
            self.raw_dir, 'DBLP4057_GAT_with_idx.mat'))
        ...
        val_mask = generate_mask_tensor(
            idx2mask(data['val_idx'][0], num_nodes))
