...
class Metapath_Relation_Network(nn.Module):
    def __init__(self,num_metapaths, num_relations, hidden_dim,out_dim,num_han_layer,num_srn_layer,
                 num_heads,num_out_heads, dropout, weight_T):
        ...
        if (num_han_layer == 1):
            self.han_layers.append(HANLayer(num_metapaths=num_metapaths, in_dim=hidden_dim, out_dim=out_dim,
                                            num_heads=num_out_heads, dropout=dropout))
        else:
            self.han_layers.append(HANLayer(num_metapaths=num_metapaths, in_dim=hidden_dim, out_dim=hidden_dim,
    ...
    def forward(self, sc_subgraphs, dst_feat, feats):
        ...
        for i in range(len(sc_subgraphs)):
            z_r.append(self.srns[i](sc_subgraphs[i],
