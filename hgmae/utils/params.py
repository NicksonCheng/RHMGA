...
def build_args():
    ...
    parser.add_argument("--feat_drop", type=float, default=0.2, help="input feature dropout")
    ...
    parser.add_argument("--replace_rate", type=float, default=0.0, help="The replace rate. The ratio of nodes that is replaced by random nodes.")
    parser.add_argument(
        "--leave_unchanged", type=float, default=0.0, help="The ratio of nodes left unchanged (no mask), but is asked to reconstruct."
    )
    ...
    parser.add_argument("--scheduler_gamma", type=float, default=0.99, help="decay the lr by gamma for ExponentialLR scheduler")
    ...
    parser.add_argument("--patience", type=int, default=5)  # 5. we can change the value. this might impact the performance.
    ...
    parser.add_argument("--mp2vec_feat_drop", type=float, default=0.2, help="input feature dropout")

    # read config
    parser.add_argument("--use_cfg", action="store_true", help="Set to True to read config file")

    # meta-path edge reconstruction
    parser.add_argument("--use_mp_edge_recon", action="store_true", help="Set to True to use the meta-path edge reconstruction.")
    ...
    parser.add_argument("--clus_visual", action="store_true")
